...
The Code-Reaver is the fierce spirit behind these scripts, a webdev master forging tools to amplify your coding workflow. Channel its expertise with the AI persona prompt in [CODE_REAVER_PROMPT.md](CODE_REAVER_PROMPT.md).

## Scripts
1. **CodeReaver-Browser-Userscript** (`codereaver-browser-userscript.js`)
   - AI-powered tooltips for code terms (e.g., "flex") and calculations (e.g., "2 + 2") in `title` or `data-tooltip` attributes, with local fallback for offline use.
   - Dynamic tooltips for code terms in `pre` and `code` elements.
   - Hover effects for code blocks to enhance readability.
2. **CodeReaver-Webdev-Tools** (`codereaver-webdev-tools.js`)
   - Responsive navigation menus and code block interactions with debounced clicks and touch support.
3. **Bookmarklet** (`codereaver-bookmarklet.js`)
   - A no-extension alternative for basic tooltip functionality, ideal for restricted environments.

## Features
- **AI Tooltips**: Explains code terms and calculations using xAI’s Grok 3 API or a local dictionary/math evaluator for offline use.
- **Dynamic Triggers**: Tooltips activate on `title`, `data-tooltip`, or code terms in `pre`/`code` elements, broadening coverage.
- **Code Block Enhancements**: Hover effects on `pre` and `code` for better focus during code reviews.
- **Responsive Navigation**: Smooth menu interactions for webdev tasks like repo browsing.
- **Mobile-Friendly**: Optimized for Android Chromium browsers with touch events and adaptive styling.
- **Bookmarklet Option**: Run basic tooltips without Tampermonkey for accessibility.
...
1. Copy the code from `codereaver-bookmarklet.js`.
2. Create a new bookmark in your browser and paste the code as the URL (prepend `javascript:`).
3. Click the bookmark on a supported site (GitHub, GitLab, etc.) to activate basic tooltips.
4. Note: Bookmarklet supports local tooltips only (no xAI API).
